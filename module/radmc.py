...
class Data:
    """Read from Athena++ vtk output, and functions to write RADMC-3D input
    files, run radmc3d to produce outputs."""
    def __init__(self, athena_filename, species_list=None, dx=4*const.pc, 
                 left_edge=None, dims=None):
        """species_list: list of chemical species to read in.
           dx: resolution of simulation. Assume cubic grids. 
           Note: over write dx with the values in file if reading h5/hdf5
           files, and only read species: CO, H2, H, e, temperature_chem.
           Note: abd and nH data is stored in Athena vtk shape: [nz, nx, ny]."""
        self.fn = athena_filename
        self.fn_ext = athena_filename.split('.')[-1]
        if self.fn_ext == "h5" or self.fn_ext == "athdf":
    ...
    def prepare(self, work_dir, image_dir, clean_outfile=True, clean_infile=True, species_coll=None,
                ...
                species_main="co", minimum_abundance=None):
        ...
        print("{} collisional species: {}".format(self.ncoll, self.species_coll))
    ...
    def write_inputs(self, ortho2para=3.):
        ...
        #o-H2 and p-H2
        fo = float(ortho2para) / float(1.0 + ortho2para)
        fp = 1. - fo
        if self.species_coll != None:
        ...
        if self.escprob == "data":
            ...
            nH0 = self.data.nH
            mu = const.mh / (1 - self.data.abd["H2"] + self.data.abd["e"] + 0.1)
            Tcs = np.copy(self.data.abd["T"])
            indxTcs = Tcs > 40.
            Tcs[indxTcs] = 40.
            cs = np.sqrt( const.kb * Tcs / mu )
            muH = 1.4271
            unit_rho = muH * const.mh #unit for density
            LJ = cs * np.sqrt(math.pi / (const.g * nH0 * unit_rho) )
        ...
        if self.gas_velocity == None:
            ...
            if self.fn_ext == "vtk":
                vel = self.data.data.read_all_data("vel")
            elif self.fn_ext == "h5" or self.fn_ext=="athdf":
                vel = self.data.vel_kms
            else:
                raise RuntimeError("file extension not recogonized: "+
                        self.fn_ext)
            unit_vel = 1.0e5 #unit for velocity
            vel0 = np.swapaxes(vel, 0, 2) * unit_vel
    ...
    def run(self, image_new_name=None, iline=1, widthkms=1, vkms=0, linenlam=40,
            levelpop_new_name=None, cmd_tail="", tausurf=False,
            incl=0, phi=0):
        ...
        " sizepc " + str(self.nx*self.dx/const.pc) + \
        " iline {:d} widthkms ".format(iline) + str(widthkms) + \
        " vkms " + str(vkms) + " linenlam " + str(linenlam) + " doppcatch " + \
        writepop + cmd_tail
        ...
        print(p.stdout.read())
        if levelpop_new_name != None:
            cmd = "mv " + self.work_dir+ "levelpop_co." + pop_ext + " " + levelpop_new_name
        p = Popen(cmd, shell=True, stdin=PIPE, stdout=PIPE, stderr=STDOUT, close_fds=True)
        print(cmd)
        print(p.stdout.read())
        return
...
class Images:
    ...
    def write_fits(self, fn_fits="image.fits", 
        ...
        primary_hdu.header.set(axis_name1.upper()+"MIN", -self.dx*self.nx/2./const.pc)
        primary_hdu.header.set(axis_name1.upper()+"MAX", self.dx*self.nx/2./const.pc)
        primary_hdu.header.set("D"+axis_name1.upper(), self.dx/const.pc)
        primary_hdu.header.set(axis_name2.upper()+"MIN", -self.dy*self.ny/2./const.pc)
        primary_hdu.header.set(axis_name2.upper()+"MAX", self.dy*self.ny/2./const.pc)
        ...
        for hdui in [primary_hdu, hdu_data]:
            ...
            hdui.header.set("CRVAL1", -self.dx*self.nx/2./const.pc)
            ...
            hdui.header.set("CRVAL2", -self.dy*self.ny/2./const.pc)
    ...
    def plotimage(self, ax, TA=True, ilam=0, **keys):
        ...
        cax = ax.imshow(np.swapaxes(imgi, 0, 1) + 1e-100,
                origin="lower", 
                extent=[-self.nx*self.dx/const.pc/2, 
                        self.nx*self.dx/const.pc/2, 
                        -self.ny*self.dy/const.pc/2, 
                        self.ny*self.dy/const.pc/2],
                        cmap="magma", **keys)
        cbar = ax.figure.colorbar(cax)
        cbar.solids.set_edgecolor("face")
        cbar.ax.set_ylabel(clabel)
        ax.set_xlabel("$x/\mathrm{pc}$", fontsize=12)
        ax.set_ylabel("$y/\mathrm{pc}$", fontsize=12)
        return
    ...
    def plotsed(self, ax, **keys):
        ...
        ax.set_xlabel(r"$v(\mathrm{km/s})$", fontsize=20)
        ax.set_ylabel(r"$\langle T_A \rangle (\mathrm{K})$", fontsize=20)
        return
    ...
    def plotimageXCO(self, ax, imgNH2, Tdect=0.6, WCOmin=0.6, extent=None,
        ...
        cbar.set_label(
            ...
            fontsize=25)
        ax.set_xlabel("$x(\mathrm{pc})$", fontsize=20)
        ax.set_ylabel("$y(\mathrm{pc})$", fontsize=20)
        return
...
class LevelPop:
    def __init__(self, fn, dims=None):
        ...
        f.close()
        return
    def getTexc(self, iline):
        """Return excitation temperature for CO J-(J-1) line."""
        Ju = iline
        nl = self.levelpop[Ju]
        nu = self.levelpop[Ju+1] + 1e-100
        T0 = 5.5 #Kelvin
        gl = 2*Ju-1
        gu = 2*Ju+1
        Texc = T0*Ju / np.log((nl/gl) / (nu/gu))
        return Texc
